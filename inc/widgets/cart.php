...
class Cart extends Base_Widget {
	...
	public function get_name() {
		return 'motors-customize-woocommerce-cart';
	}

	public function get_title() {
		return esc_html__( 'Cart Customize', 'elementor-pro' );
	}
	...
	public function get_keywords() {
		return [ 'woocommerce', 'cart', 'motors' ];
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'edit_button',
			[
				'raw' => sprintf( '<a href="#" target="_blank" class="elementor-button elementor-edit-template" style="margin-top:0;"><i class="eicon-pencil"> %s</i></a>', esc_html__( 'Edit Template', 'elementor-pro' ) ),
        ...
        $this->add_control(
            'sections_typography_options',
            [
                'type' => Controls_Manager::HEADING,
                'label' => esc_html__( 'Options', 'motors-child' ),
            ]
        );

        $this->add_control(
            'sections_option_color',
            [
                'label' => esc_html__( 'Color', 'elementor-pro' ),
                'type' => Controls_Manager::COLOR,
                'selector' => '{{WRAPPER}} .stm-motors-woocommerce-cart__option--title'
            ]
        );
        ...
        $this->add_group_control(
            Group_Control_Typography::get_type(),
            [
                'name' => 'sections_options_typography',
                'selector' => '{{WRAPPER}} .stm-motors-woocommerce-cart__option--title',
            ]
        );
        ...
        $this->add_group_control(
            Group_Control_Text_Shadow::get_type(),
            [
                'name' => 'sections_options_text_shadow',
                'selector' => '{{WRAPPER}} .stm-motors-woocommerce-cart__option--title',
            ]
        );
    ...
    {
        wp_enqueue_style( 'single-car-woo-cart', get_stylesheet_directory_uri() . '/assets/css/single-car-woo-cart.css', array(), '' );
        wp_enqueue_script( 'single-car-woo-cart', get_stylesheet_directory_uri() . '/assets/js/single-car-woo-cart.js', array(), '' );
    }
	...
	public function render() {
		...
		if ( $this->has_empty_cart_template() && WC()->cart->get_cart_contents_count() === 0 ) {
            ...
            if ( apply_filters( 'woocommerce_output_cart_shortcode_content', true ) ) {
                ob_start();
                stm_listings_load_template('woocommerce/cart');
